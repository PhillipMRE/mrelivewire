    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="Agent" format="csv" />
                <livewire:excel-export model="Agent" format="xlsx" />
                <livewire:excel-export model="Agent" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        ...
                        <th>
                            ...
                            @include('components.table.sort', ['field' => 'is_vetted'])
                        </th>
                        <th>
                        </th>
                    </tr>
                ...
                <tbody>
                    @forelse($agents as $agent)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $agent->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $agent->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('agent_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.agents.show', $agent) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('agent_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.agents.edit', $agent) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('agent_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $agent->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
